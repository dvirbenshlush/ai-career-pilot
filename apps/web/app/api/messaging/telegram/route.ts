  ...
  if (action === 'scan') {
    ...
    try {
      ...
      if (user) {
        ...
        console.log(`[Telegram] passing ${knownFingerprints.length} known fingerprints to messaging service`)
      }
    } catch { /* non-fatal — scan still runs without cache */ }

    const { ok, status, text } = await proxyPost('/telegram/scan', { botToken, channels, userProfile, maxAgeDays, knownFingerprints })
